...
class Auth extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->library(array('authit', 'session'));
		$this->load->helper(array('authit', 'url'));
	...
	public function login()
	{
		...
		if($this->form_validation->run()){
			if($this->authit->login(set_value('email'), set_value('password'), set_value('rememberme'))){
				// Redirect to your logged in landing page here
				redirect('dashboard');
			} else {
			    $flashmsg = array(
		...
		if($this->form_validation->run()){
			if($this->authit->signup(set_value('email'), set_value('password'))){
			    ...
			    $this->session->set_flashdata(array(
			            'class'     => 'alert alert-danger',
                        'message'   => 'Failed to sign up with the given email address and password.'
                ));
			}
		}
...
Note: This reset code will expire after '. date('j M Y') .'.';
            ...
            $this->session->set_flashdata(array(
                    ...
                    'message'   => 'Thank you. We have sent you an email with further instructions on how to reset your password.'
            ));
		}
